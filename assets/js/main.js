...
function getWeatherData() {
    ...
    if(textContentArticle === "") {
        ...
        fetch(`https://api.openweathermap.org/data/2.5/weather?q=${inputCity.value}&units=metric&appid=2fa1662c1af641d6fd162fc1fe68f4f5`)
...
function createHtmlCard(weatherDataHeader, weatherDataBody) {
    ...
    const formatInputCity = inputCity.slice(0, 1).toUpperCase() + inputCity.slice(1);
    ...
    weatherDataBody.forEach((data) => {
        tableData.innerHTML += `<tr><td>${Object.keys(data)[0]}</td><td>${Object.values(data)[0]}</td></tr>`;
})
}
...
function obtainedTime(unixTime) {
    ...
    const obtainNew = "Obtained at " + obtain.getHours() + ":" + obtain.getMinutes() + " Uhr, " + obtain.getDate() + "." + (obtain.getMonth()+1) + "." + obtain.getFullYear();
    return obtainNew;
}
...
function localeTime(timezone) {
    ...
    return utcTime.getUTCHours() + timeDiffInHours + ":" + utcTime.getUTCMinutes() + " Uhr";
}
